<?php
session_start();
$pass='auto'; // Пароль для входа в Админку
...
if((isset($_POST['slovo'])||isset($_POST['sekret']))||($_SESSION['sekret']==md5($pass))){
	if (($_POST['slovo']==$pass)||($_SESSION['sekret']==md5($pass))){
		...
		$adm=1;
			...
			<body>
			<center><form method="POST" action="1.php" style="margin-top: 30px;">
...
<body>
<div id="menu">
<form action="1.php" id="myform" method="POST">
...
<a href="1.php?mode=0">Текст</a>
<a href="1.php?mode=7">Картинки</a>
<a href="1.php?mode=5">HTML</a>
<a href="1.php?mode=8">CSS и JS</a>
<a href="1.php">Помощь</a>
...
if($_GET['mode']=='7'){
	...
	for ($j=0; $j< count($imgmas[1]); $j++) {
		...
		echo('<div class="kartinka"><a href="1.php?mode=1&img='.$imgname.'"><img src="'.$imgname.'?'.rand(1, 32000).'"></a><br>'.$imgname.'<br>');
...
if($_GET['mode']=='1'){
	...
	echo('<form enctype="multipart/form-data" action="1.php?mode=2&img='.$imgname.'" method="POST">Загрузить картинку с компьютера: <p><input name="userfile" type="file" required><p><input type="submit" style="width: 300px;
    padding-top: 19px;
    padding-bottom: 22px;
    background-color: #005bff;
    box-shadow: 0 20px 18px -10px rgba(0, 91, 255, 0.43);
...
if($_GET['mode']=='2'){
	...
	if (move_uploaded_file($_FILES['userfile']['tmp_name'], $imgname)) {
		echo "<br><br><center>Файл был успешно загружен.<p><a href='1.php'>Вернуться к списку картинок</a><p>Что бы увидеть изменения на сайте, обновите страницу в гугл хроме с зажатей клавишей shift (не эту (это админка), а страницу Вашего сайта)";
	};
};
...
if($_GET['mode']=='0'){
	...
	for ($j=0; $j< count($ff); $j++) { 
		echo('<a href="1.php?mode=3&j='.$j.'" class="mytext">'.$ff[$j].'</a>');
	};
};
...
if($_GET['mode']=='3'){
	...
	echo('<div style="margin: 0 auto; text-align: center;"><form method="POST" action="1.php?mode=4&j='.$jj.'"><br><br><h2>Редактирование текстового фрагмента</h2><br><br><textarea name="mytext">'.$tektekst.'</textarea><br><input style="width: 300px;
    padding-top: 19px;
    padding-bottom: 22px;
    background-color: #005bff;
    box-shadow: 0 20px 18px -10px rgba(0, 91, 255, 0.43);
...
if($_GET['mode']=='4'){
	...
	echo "<br><br><center>Текст был успешно изменен.<p><a href='1.php?mode=0'>Вернуться к списку текстов</a><p>Что бы увидеть изменения на сайте, обновите страницу (не эту (это админка), а страницу Вашего сайта)";
};
...
if($_GET['mode']=='5'){
	...
	echo('<div style="margin: 0 auto; text-align: center;"><form method="POST" action="1.php?mode=6"><br><br><h2>Редактирование HTML кода</h2><br><br><textarea name="mytext" style="width: 90%; height: 500px;">'.$template.'</textarea><br><input style="width: 300px;
    padding-top: 19px;
    padding-bottom: 22px;
    background-color: #005bff;
    box-shadow: 0 20px 18px -10px rgba(0, 91, 255, 0.43);
...
if($_GET['mode']=='8'){
	...
	$rrr=trim($cssmas[1][$j]);
	if (!(strstr($rrr, "http"))) {
 	echo('<a class="cssjs" href="1.php?mode=9&fl='.$rrr.'">'.$rrr.'</a><p>');
	};
	};
	$cssreg = "/[\"']((.*\\/\\/|)([\\/a-z0-9_%]+\\.(js)))[\"']/"; 
	preg_match_all($cssreg, $template, $cssmas);
...
if($_GET['mode']=='9'){
	...
	echo('<div style="margin: 0 auto; text-align: center;"><form method="POST" action="1.php?mode=10&fl='.$_GET['fl'].'"><br><br><h2>Редактирование кода</h2><br><br><textarea name="mytext" style="width: 90%; height: 500px;">'.$template.'</textarea><br><input style="width: 300px;
    padding-top: 19px;
    padding-bottom: 22px;
    background-color: #005bff;
    box-shadow: 0 20px 18px -10px rgba(0, 91, 255, 0.43);
...
<br><br><br><p><a href="http://artstranger.ru/cms.html" target="_blank">Видео инструкция по админке тут —  просто нажмите по этому тексту.</a><br><br>
		<p>Есть вопросы? Нужно что бы изминения внес я? <p>Тогда <a href="http://lenadiz.ru" target="_blank">вот мой сайт — lenadiz.ru просто нажмите по этому тексту.</a> <p>Так же, можете написать мне на почту lenadizru@gmail.com</div>');
};
